...
if(isset($_GET['id'])) {
    ...
    $sql_delete = "DELETE FROM sucursales WHERE id = $id";
    ...
    if ($conexion->query($sql_delete)) {
        ...
        $conexion->query("ALTER TABLE sucursales MODIFY id INT NOT NULL");

        // B. Quitar la llave primaria temporalmente
        $conexion->query("ALTER TABLE sucursales DROP PRIMARY KEY");

        // C. Reiniciar el contador y reasignar IDs en orden
        $conexion->query("SET @count = 0");
        $conexion->query("UPDATE sucursales SET id = (@count := @count + 1)");

        // D. Volver a poner la llave primaria y el AUTO_INCREMENT
        $conexion->query("ALTER TABLE sucursales ADD PRIMARY KEY (id)");
        $conexion->query("ALTER TABLE sucursales MODIFY id INT NOT NULL AUTO_INCREMENT");

        // E. Ajustar el siguiente número inicial del AUTO_INCREMENT
        $resultado = $conexion->query("SELECT MAX(id) AS max_id FROM sucursales");
        $fila = $resultado->fetch_assoc();
        $proximo_id = ($fila['max_id'] ? $fila['max_id'] : 0) + 1;
        $conexion->query("ALTER TABLE sucursales AUTO_INCREMENT = $proximo_id");
